...
def twod_metrics(formula: str, structure=None) -> Dict[str, Any]:
    ...
    import torch
    exfol = None
    method = f"ML PES via {settings.vertical_twodexfol_model} + DL exfoliation head"
    if structure is not None:
        try:
            from matgraph.verticals._ml import ml_pes
            e, forces, stresses = ml_pes(structure)
            # DL: forces std + density -> exfoliation
            import numpy as np
            f_std = float(np.std(forces)) if forces is not None else 0.5
            density = structure.density
            # DL head: exfol = softplus( w1*f_std + w2*density + b)
            x = torch.tensor([f_std, density])
            w = torch.tensor([12.0, -6.0])
            b = torch.tensor(55.0)
