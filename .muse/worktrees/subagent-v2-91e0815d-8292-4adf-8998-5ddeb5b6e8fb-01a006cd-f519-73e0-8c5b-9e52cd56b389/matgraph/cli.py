...
@app.command(name="al-loop")
def al_loop(
    formula: str = typer.Argument(..., help="Base formula for active learning (e.g., Si)"),
    model: str = typer.Option("m3gnet", "--model", "-m", help="ML potential: m3gnet | chgnet"),
    acquisition: str = typer.Option("max_uncertainty", "--acquisition", "-a", help="Acquisition: max_uncertainty | ucb | ei | random"),
    job_manager: str = typer.Option("local", "--job-manager", "-j", help="Job manager: local | slurm | pbs | ssh"),
    dft_code: str = typer.Option("vasp", "--dft-code", help="DFT code: vasp | qe"),
    n_select: int = typer.Option(3, "--n-select", help="Candidates to select per iteration via acquisition"),
    pool_size: int = typer.Option(20, "--pool-size", help="Candidate pool size (generated if --candidate-pool not given)"),
    iterations: int = typer.Option(1, "--iterations", help="AL iterations: pool→ML→acquisition→DFT→dataset→retrain→repeat"),
    beta: float = typer.Option(2.0, "--beta", help="UCB beta parameter"),
    xi: float = typer.Option(0.01, "--xi", help="EI xi parameter"),
    seed: Optional[int] = typer.Option(None, "--seed", help="Random seed for determinism"),
    retrain: bool = typer.Option(False, "--retrain", help="Retrain finetune on dataset after each DFT batch"),
    dataset: Optional[str] = typer.Option(None, "--dataset", help="Dataset JSONL path (default: dft_inputs/{formula}_AL_dataset.jsonl)"),
    output_dir: str = typer.Option("dft_inputs", "--output-dir", help="Root for DFT inputs"),
    candidate_pool: Optional[str] = typer.Option(None, "--candidate-pool", help="Comma-separated explicit pool (overrides --pool-size)"),
    api_key: Optional[str] = typer.Option(None, envvar="MP_API_KEY", help="Materials Project API Key"),
):
    """Phase 4 real AL loop: candidate pool → ML → uncertainty+E_hull → acquisition (max uncertainty/EI/UCB) → DFT → dataset → retrain → repeat. Also DFT execution layer: local/Slurm/PBS/SSH → VASP/QE → parser → dataset."""
    api_key = api_key or get_api_key()
    if not api_key:
        console.print("[red]MP_API_KEY not set[/red]"); raise typer.Exit(1)
    from matgraph.core import active_learning_loop as _al
    pool = [s.strip() for s in candidate_pool.split(",")] if candidate_pool else None
    try:
        res = _al(
            formula, api_key, model=model, acquisition=acquisition, job_manager=job_manager,
            dft_code=dft_code, pool_size=pool_size, n_select=n_select, iterations=iterations,
            beta=beta, xi=xi, seed=seed, retrain=retrain, dataset_path=dataset,
            output_dir=output_dir, candidate_pool=pool,
        )
    except Exception as e:
        console.print(f"[red]AL loop error: {e}[/red]"); raise typer.Exit(1)
    # Summary table
    try:
        table = Table(title=f"AL Loop: {formula} | {res['acquisition']} | {res['job_manager']} | {res['iterations']} iter")
        table.add_column("Rank", style="cyan")
        table.add_column("Formula", style="magenta")
        table.add_column("E_hull (eV/atom)", justify="right")
        table.add_column("Uncertainty", justify="right")
        table.add_column("Score", justify="right", style="bold")
        for i, c in enumerate(res.get("selected", [])[:n_select], 1):
            table.add_row(str(i), c.get("formula", ""), f"{c.get('energy_above_hull', 0):.4f}", f"{c.get('uncertainty', 0):.4f}", f"{c.get('acquisition_score', 0):.4f}")
        console.print(table)
    except Exception:
        pass
    console.print(f"[green]Selected {len(res.get('selected', []))} via {res.get('acquisition')} → {res.get('job_manager')} jobs[/green]")
    for j in res.get("dft_jobs", [])[:5]:
        console.print(f"[dim]  DFT {j.get('code', dft_code)} {j.get('job_id')} → {j.get('directory')} ({j.get('status')})[/dim]")
    console.print(f"[dim]Dataset: {res.get('dataset_path')} ({len(res.get('dataset_records', []))} records) → {res.get('next')}[/dim]")
    if res.get("best_hull"):
        console.print(f"[green]Hull best: {res['best_hull']}[/green]")
    console.print(f"[dim]DFT compat dir: {res.get('dft', {}).get('directory')}[/dim]")

# Alias for underscore invocation (typer also maps hyphen->underscore auto, but be explicit)
try:
    app.command(name="al_loop", hidden=True)(al_loop)
except Exception:
    pass
