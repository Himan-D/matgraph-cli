...
class MatGraphSDK:
    ...
    def active_learning_loop(self, formula: str, acquisition: str = "max_uncertainty", job_manager: str = "local", **kwargs) -> Dict[str, Any]:
        """Phase 4 AL loop via SDK.

        Args:
            formula: base formula
            acquisition: max_uncertainty | ucb | ei | random
            job_manager: local | slurm | pbs | ssh
            **kwargs: pool_size, n_select, iterations, beta, xi, dft_code, seed, retrain, etc.
        """
        from matgraph.core import active_learning_loop as _al
        return _al(formula, self.api_key, acquisition=acquisition, job_manager=job_manager, **kwargs)

    # alias
    def al_loop(self, *args, **kwargs):
        return self.active_learning_loop(*args, **kwargs)
