...
def stability(
    formula: str,
    model: str = typer.Option("m3gnet", "--model", help="ML model: m3gnet or chgnet (legacy megnet/cgcnn alias to m3gnet)"),
    uncertainty: bool = typer.Option(False, "--uncertainty", help="Include uncertainty ±σ, confidence and 95% CI"),
    json_output: bool = typer.Option(False, "--json", help="Output raw JSON"),
    api_key: str = typer.Option(None, envvar="MP_API_KEY"),
):
    """
    Check thermodynamic stability (convex hull distance) — E_hull is primary abstraction.

    Uses ML convex hull engine (M3GNet/CHGNet) with structure validation,
    competing phases, and optional uncertainty. Returns
    {energy_above_hull, stability, model, uncertainty, confidence, ci_95}.

    Example: matgraph stability LiFePO4 --model chgnet --uncertainty
    """
    # Phase 1: uncertainty-aware ML engine is the primary path
    # Falls back to SDK stability_hull for polymorph listing when needed.
    try:
        from matgraph.core import predict_stability
        result = predict_stability(formula, model=model, with_uncertainty=uncertainty, uncertainty=uncertainty, api_key=api_key)
        if json_output:
            import json as _json
            # ensure output matches contract precisely
            out = {
            ...
            return
        # rich table for human
        table = Table(title=f"ML Convex Hull Stability: {formula} [{result['model']}]")
        table.add_column("Formula", style="magenta")
        table.add_column("E_above_hull (eV/atom)", justify="right", style="blue")
        table.add_column("Stability", justify="center")
        table.add_column("Model", style="cyan")
        if uncertainty:
            table.add_column("±σ (eV/atom)", justify="right", style="yellow")
            table.add_column("Confidence", justify="right", style="green")
            table.add_column("CI 95%", justify="right")
            color = "green" if result["stability"] == "Stable" else ("yellow" if result["stability"] == "Metastable" else "red")
            ci_str = f"[{result['ci_95'][0]:.3f}, {result['ci_95'][1]:.3f}]"
            table.add_row(
                formula,
                f"{result['energy_above_hull']:.4f}",
                f"[{color}]{result['stability']}[/{color}]",
                result["model"],
                f"{result['uncertainty']:.3f}",
        ...
        console.print(table)
        if uncertainty:
            console.print(f"[dim]E_hull = {result['energy_above_hull']:.4f} ± {result['uncertainty']:.3f} eV/atom | confidence={result['confidence']:.2f} | 95% CI {result['ci_95']}[/dim]")
        # Also optionally show competing phases via SDK when API key present
        # (non-fatal)
        try:
            sdk = MatGraphSDK(api_key=api_key)
            # try to list polymorphs for context, but don't fail
            poly = sdk.stability(formula)
            if poly and len(poly) > 1:
                console.print(f"[dim]Found {len(poly)} polymorphs in MP (reference). Use --json for ML result alone.[/dim]")
        except Exception:
            pass
    except Exception as e:
        # fallback to legacy polymorph listing
        try:
            sdk = MatGraphSDK(api_key=api_key)
            ...
            console.print(f"[bold red]Error: {e} / fallback: {e2}[/bold red]")
