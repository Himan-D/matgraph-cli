...
@app.command()
def benchmark(
    formula: str = typer.Option("Si", "--formula", "-f", help="Formula to benchmark"),
    model: str = typer.Option("m3gnet", "--model", "-m", help="Model to use"),
    test_size: float = typer.Option(0.2, "--test-size", help="Test split fraction"),
    time_split: bool = typer.Option(False, "--time-split", help="Use time-split (material_id sort) instead of random"),
    split: Optional[str] = typer.Option(None, "--split", help="Split strategy: random|element|chemsys|system|prototype|temporal|ood|all"),
    ood: bool = typer.Option(False, "--ood", help="Also report Element-OOD and System-OOD gaps"),
):
    """Benchmark: splits random/element/chemsys/prototype/temporal/OOD → MAE/RMSE/R2/Spearman/ROC-AUC/calibration/OOD.

    Reports a table with Random / Element-OOD / System-OOD (plus Temporal/Prototype when --split all).
    Chemical-system-disjoint split ensures no chemsys overlap between train/test.
    """
    api_key = get_api_key()
    if not api_key:
        console.print("[red]MP_API_KEY not set[/red]")
        raise typer.Exit(1)
    ...
    try:
        ...
        if len(results) < 2:
            console.print(f"[yellow]Only {len(results)} polymorphs, need ≥2 — using all[/yellow]")
        # filter to pairs for display
        from matgraph.evals.benchmark import (
            time_split_benchmark,
            random_split_benchmark,
            chemical_system_split,
            element_ood_split,
            prototype_split,
            benchmark_report,
            format_report_table,
        ...
        # If legacy --time-split used, honor it as temporal split
        if time_split and not split:
            split = "temporal"

        if split is None or split == "all" or ood:
            report = benchmark_report(results, test_size=test_size)
            # Render rich table Random/Element-OOD/System-OOD
            table = _Table(title=f"Benchmark Report: {formula} ({model}) — Random / Element-OOD / System-OOD")
            ...
            for name in ["Random", "Element-OOD", "System-OOD"]:
                m = report.get(name, {})
                if "error" in m:
                    table.add_row(name, f"error:{m.get('error')}", "-", "-", "-", "-", "-", str(m.get("n", 0)))
                else:
                    def fmt(v, nd=3):
                        import math
                        try:
            ...
            # also show OOD gaps
            try:
                console.print(f"[dim]OOD gap Element-OOD - Random MAE: {report.get('OOD_gap_Element_vs_Random', float('nan')):.4f}[/dim]")
                console.print(f"[dim]OOD gap System-OOD - Random MAE: {report.get('OOD_gap_System_vs_Random', float('nan')):.4f}[/dim]")
            except Exception:
                pass
            # markdown fallback for logs
            console.print("[dim]" + format_report_table(report) + "[/dim]")
            # also log metrics
            try:
                from matgraph.tracking import init
                run = init(project="benchmark", name=f"{formula}-{model}", config={"formula": formula, "model": model, "test_size": test_size, "split": split or "all", "n": len(results)})
                for k in ["Random", "Element-OOD", "System-OOD"]:
                    m = report.get(k, {})
                    if "error" not in m:
                        run.log({f"{k}/mae": m.get("mae"), f"{k}/rmse": m.get("rmse"), f"{k}/r2": m.get("r2"), f"{k}/spearman": m.get("spearman"), f"{k}/roc_auc": m.get("roc_auc"), f"{k}/ece": m.get("ece")})
                run.finish()
            except Exception:
                pass
        elif split in ("random",):
            from sklearn.metrics import mean_absolute_error, mean_squared_error, r2_score
            m = random_split_benchmark(results, test_size=test_size)
            if "error" in m:
                console.print(f"[red]{m['error']}[/red]")
                raise typer.Exit(1)
            console.print(f"[green]Random MAE: {m['mae']:.4f} RMSE: {m['rmse']:.4f} R2: {m['r2']:.3f} Spearman: {m['spearman']:.3f} ROC-AUC: {m['roc_auc']:.3f} ECE: {m['ece']:.3f} (n={m['n_test']})[/green]")
        elif split in ("element", "element-ood", "element_ood"):
            m = element_ood_split(results, test_size=test_size)
            if "error" in m:
                console.print(f"[red]{m['error']}[/red]")
                raise typer.Exit(1)
            console.print(f"[green]Element-OOD MAE: {m['mae']:.4f} RMSE: {m['rmse']:.4f} R2: {m['r2']:.3f} Spearman: {m['spearman']:.3f} ROC-AUC: {m['roc_auc']:.3f} ECE: {m['ece']:.3f} (n={m['n_test']}) held_out={m.get('held_out_elements')}[/green]")
        elif split in ("chemsys", "chemsystem", "system", "system-ood", "system_ood", "chem_system"):
            m = chemical_system_split(results, test_size=test_size)
            if "error" in m:
                console.print(f"[red]{m['error']}[/red]")
                raise typer.Exit(1)
            console.print(f"[green]System-OOD MAE: {m['mae']:.4f} RMSE: {m['rmse']:.4f} R2: {m['r2']:.3f} Spearman: {m['spearman']:.3f} ROC-AUC: {m['roc_auc']:.3f} ECE: {m['ece']:.3f} (n={m['n_test']} chemsys_test={m.get('n_chemsys_test')})[/green]")
        elif split in ("prototype", "prototype-ood"):
            m = prototype_split(results, test_size=test_size)
            if "error" in m:
                console.print(f"[red]{m['error']}[/red]")
                raise typer.Exit(1)
            console.print(f"[green]Prototype-OOD MAE: {m['mae']:.4f} RMSE: {m['rmse']:.4f} R2: {m['r2']:.3f} (n={m['n_test']})[/green]")
        elif split in ("temporal", "time", "time_split"):
            m = time_split_benchmark(results, test_size=test_size)
            if "error" in m:
                console.print(f"[red]{m['error']}[/red]")
                raise typer.Exit(1)
            console.print(f"[green]Temporal MAE: {m['mae']:.4f} RMSE: {m['rmse']:.4f} R2: {m['r2']:.3f} (n={m['n_test']}) stable_true={m['stable_true']} stable_pred={m['stable_pred']}[/green]")
        else:
            console.print(f"[red]Unknown split '{split}'. Use random|element|chemsys|prototype|temporal|ood|all[/red]")
            raise typer.Exit(1)
    except SystemExit:
        raise
    except Exception as e:
