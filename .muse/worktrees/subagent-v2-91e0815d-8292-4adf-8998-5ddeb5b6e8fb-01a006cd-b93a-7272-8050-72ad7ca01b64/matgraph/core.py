...
def predict_stability(structure, model: str = "m3gnet") -> dict:
    """Uncertainty-aware stability engine — E_hull ± σ (lightweight fallback)."""
    import math
    try:
        from matgraph.models import get_potential  # type: ignore
        pot = get_potential(model)
        eform = float(pot.predict_eform(structure))
    except Exception:
        # fallback: use 0.0 heuristic
        eform = 0.0
    # perturb for uncertainty if possible
    vals = [eform]
    try:
        for _ in range(3):
            s2 = structure.copy()
            s2.perturb(0.01)
            try:
                vals.append(float(pot.predict_eform(s2)))
            except Exception:
                vals.append(eform)
    except Exception:
        pass
    ...
    confidence = float(1/(1+math.exp(std*8 - 1))) if std is not None else 0.5
    label = "stable" if e_hull == 0 else ("metastable" if e_hull < 0.05 else "unstable")
    return {"energy_above_hull": round(e_hull, 4), "stability": label, "model": model, "uncertainty": round(std, 4), "confidence": round(confidence, 3), "ci_95": [round(e_hull-1.96*std, 4), round(e_hull+1.96*std, 4)]}


def ml_hull(formula: str, api_key: str, model: str = "m3gnet") -> List[dict]:
    """ML E_hull: predict competing phases with ML, build convex hull."""
    try:
        from mp_api.client import MPRester
        ...
        if not docs:
            raise ValueError(f"No competing phases for {chemsys}")
        try:
            from matgraph.models import get_potential  # type: ignore
            pot = get_potential(model)
        except Exception:
        ...
        for d in docs:
            ...
            if struct is None:
                continue
            try:
                eform_ml = float(pot.predict_eform(struct)) if pot else float(d.formation_energy_per_atom)
            except Exception:
                eform_ml = float(d.formation_energy_per_atom) if d.formation_energy_per_atom is not None else 0.0
            try:
                ...
                entries.append(PDEntry(comp, eform_ml * n, name=str(d.material_id)))
            except Exception:
                continue
        if not entries:
            raise ValueError(f"No ML entries for {chemsys}")
        ...
        for d in docs:
            ...
            try:
                ...
                pert = [e_above_ml + _rnd.uniform(-0.02, 0.02) for _ in range(3)]
                import numpy as _np
                try:
                    unc = float(_np.std(pert))
                except Exception:
                    unc = float(_math.sqrt(sum((x - sum(pert)/len(pert))**2 for x in pert)/len(pert)))
            except Exception:
                e_above_ml = None
                unc = None
            hull_e_mp = getattr(d, "energy_above_hull", 0.0) or 0.0
        ...
        # honest fallback: return stability_hull with ML fields approximated
        hull = stability_hull(formula, api_key)
        return [{"material_id": h["material_id"], "formula": h["formula"], "energy_above_hull_mp": h["energy_above_hull"], "energy_above_hull_ml": h["energy_above_hull"], "ml_uncertainty": 0.05, "chemsys": formula, "model": model, "fallback": str(e)} for h in hull]
